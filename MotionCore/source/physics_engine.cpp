...
void PhysicsEngine::SetBox(const Tbx::Vector3f& _extent)
{
	m_BoxExtent = _extent;
}
...
uint32_t PhysicsEngine::CreateCircleBody(Tbx::Vector2f _position, float radius, float mass)
{
	Particule p;
...
void MotionCore::PhysicsEngine::AddForce(uint32_t _id, Tbx::Vector2f _force)
{
...
MOTION_CORE void MotionCore::PhysicsEngine::AddImpulse(uint32_t _id, Tbx::Vector2f _impulse)
{
	auto it = std::find_if(m_Instance->particles.begin(), m_Instance->particles.end(),
		[_id](const Particule& p) { return p.body.id == _id; });
...
Tbx::Vector2f MotionCore::PhysicsEngine::GetCircleBodyPosition(uint32_t _id)
{
	auto it = std::find_if(m_Instance->particles.begin(), m_Instance->particles.end(),
		[_id](const Particule& p) { return p.body.id == _id; });
...
{
	const Tbx::Vector3f& boxExtend = m_BoxExtent;
	...
	while (!m_Instance->collisionPoint.empty())
	{
		...
		Tbx::Vector2f impulseNormalB1 = collisionPoint.normal * impulseB2;
		Tbx::Vector2f impulseNormalB2 = -collisionPoint.normal * impulseB2;
...
void MotionCore::PhysicsEngine::AddForce(Body* _body, Tbx::Vector2f _force)
{
	_body->force += _force;
}

void MotionCore::PhysicsEngine::AddImpulse(Body* _body, Tbx::Vector2f _impulse)
{
